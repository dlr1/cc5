...
export class CSSDevice extends NitDevice {
    ...
    private async getCSSCiscoPanelDetails(ring, device, selectedRing, command, parser) {
        ...
        for (var i = 0; i < 2; i++) {
            ...
            var data = await this.serviceHelper.sendCommand({ session_key: device.sessionKey, commands: [{ command_string: cmd, name: i }] })
            if (data["error"] != null)
    ...
    async getCSSAlcatelData(ring, device, selectedRing) {
        ...
        if (data.error == null) {
            ...
            try {
                ...
                await this.serviceHelper.disconnect({ session_key: device.sessionKey || '' });
            }
        }
        else {
            await this.serviceHelper.disconnect({ session_key: device.sessionKey || '' });
